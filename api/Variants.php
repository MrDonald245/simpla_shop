...
class Variants extends Simpla
{
	...
	*/
	...
	{		
		...
		$query = $this->db->placehold("SELECT v.id, v.product_id , v.price, NULLIF(v.compare_price, 0) 
                    as compare_price, v.sku, v.new_post_weight, IFNULL(v.stock, ?) 
                    as stock, (v.stock IS NULL) 
                    as infinity, v.name, v.attachment, v.position
	...
	{	
		...
		$query = $this->db->placehold("SELECT v.id, v.product_id , v.price, NULLIF(v.compare_price, 0) 
                    as compare_price, v.sku, v.new_post_weight, IFNULL(v.stock, ?) 
                    as stock, (v.stock IS NULL) as infinity, v.name, v.attachment

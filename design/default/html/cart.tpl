...
<ul id="deliveries">
    ...
    {if $settings->new_post_key}
        <li>
            ...
            <div class="order_weight" style="display: none;" weight="{$cart->total_weight|convert}"></div>
            <div class="checkbox">
                <input type="radio" name="delivery_id" value="999999" id="deliveries_new_post">
            </div>
            ...
            <div class="block_adress" style="display: none">
                <select name="new_post_recipient_city" class="simpla_inp">
                    {if $new_post_city}
                        <option>Выбор города</option>
                        {foreach $new_post_city as $city}
                            <option ref="{$city['Ref']}"  value="{$city['DescriptionRu']}">{$city['DescriptionRu']}</option>
                        {/foreach}
                    {else}
                ...
                <select name="new_post_recipient_ware_house" class="simpla_inp">
                    {if $new_post_ware_house_list}
                        {foreach $new_post_ware_house_list as $war_house}
                            <option  ref="{$city['Ref']}"  value="{$war_house['DescriptionRu']}">{$war_house['DescriptionRu']}</option>
                        {/foreach}
                    {else}

...
<form method=post id=order enctype="multipart/form-data">
    ...
    <div id="name">
        ...
        <a href="{url view=print id=$order->id}" target="_blank"><img src="./design/images/printer.png" name="export"
    ...
    <div id="order_details">
        <h2>Детали заказа <a href='#' class="edit_order_details"><img src='design/images/pencil.png' alt='Редактировать'
        ...
        <div class='layer'>
            <h2>Покупатель <a href='#' class="edit_user"><img src='design/images/pencil.png' alt='Редактировать'
                    ...
                    <a href="#" class='delete_user'><img src='design/images/delete.png' alt='Удалить'
                                                         title='Удалить'></a>
                {/if}</h2>
            <div class='view_user'>
                {if !$user}
        ...
        <div class='layer'>
            <h2>Примечание <a href='#' class="edit_note"><img src='design/images/pencil.png' alt='Редактировать'
                                                              title='Редактировать'></a></h2>
            <ul class="order_details">
                <li>
    ...
    <div id="purchases">
		    	...
		    	{if !$purchase->variant}
                    ...
                    {foreach $purchase->product->variants as $v}
                        {if $v->stock>0 || $v->id == $purchase->variant->id}
                            <option price='{$v->price}' amount='{$v->stock}' value='{$v->id}'
                    ...
                    <div class="icons cell">
                        {if !$order->closed}
                            ...
                            {elseif !$purchase->variant}
                                <img src='design/images/error.png' alt='Вариант товара был удалён'
        ...
        <div id="add_purchase" {if $purchases}style='display:none;'{/if}>
            <input type=text name=related id='add_purchase' class="input_autocomplete"
        ...
        <div class="subtotal layer">
            С учетом
            купона<b> {($subtotal-$subtotal*$order->discount/100-$order->coupon_discount)|round:2} {$currency->sign}</b>
        </div>
        ...
        <div class="block delivery">
            ...
            <select name="delivery_id">
                ...
                {/foreach}
                {if $settings->new_post_key}
                    <option value="999999" {if 999999 == $delivery_id}selected{/if}>Новая почта</option>
                {/if}
            </select>
            ...
            <div class="separate_delivery">
                <input type=checkbox id="separate_delivery" name=separate_delivery value='1'
                       {if $order->separate_delivery}checked{/if}> <label for="separate_delivery">оплачивается
                    отдельно</label>
            </div>
            {if $settings->new_post_key}
                <div class="new_post">
                    {if 999999 == $delivery_id}
                        <div class="block layer new_post">
                            <h2>"Новая почта"</h2>
                            <script src="/design/{$settings->theme|escape}/js/new_post_api.js"></script>
                            <ul>
                                <li>
                                    <div style="margin-bottom: 6px;"><span class="add_en button_green"
                                ...
                                <li class="reci_d">
                                    <label for="new_post_last_name">Фамилия:</label>
                                    <input id="new_post_last_name" name="new_post_last_name"
                                           type="text" {if $order->new_post_last_name} value="{$order->new_post_last_name}"{/if}>

                                </li>
                                <li class="reci_d">
                                    <label for="new_post_name">Имя:</label>
                                    <input id="new_post_name" name="new_post_name"
                                           type="text" {if $order->new_post_name} value="{$order->new_post_name}"{/if}>

                                </li>
                                <li class="reci_d">
                                    <label for="new_post_midle_name">Отчество:</label>
                                    <input id="new_post_midle_name" name="new_post_midle_name"
                                           type="text" {if $order->new_post_midle_name} value="{$order->new_post_midle_name}"{/if}>

                                </li>
                                <li>
                                    <span>Вес посылки задать:</span>
                                    <input name="new_post_custom_weight"
                                           type="text" {if $order->new_post_custom_weight} value="{$order->new_post_custom_weight}"{/if}>
                                    <input name="new_post_default_weight"
                                           type="hidden" {if $purchases_weight} value="{$purchases_weight}"{/if}>
                                </li>
                                {if $purchases_weight > 10}
                                    <li>
                                        <div class="warning weight">Груз не может быть доставлен в некоторые отделения
                                            (вес груза :{$purchases_weight} кг.)
                                        </div>
                                    </li>
                                {/if}
                                {if $order->new_post_document_id}
                                    <li class="reci_d new_post_remove">
                                        <div class="add_en_number"><label>Накладная:</label>
                                            <span><b><a target="_blank"
                                                        href="https://my.novaposhta.ua/orders/index/OrderField/DateTime/OrderDirection/DESC">{$order->new_post_document_id}</a></b></span>
                                            <input name="new_post_document_id" value="{$order->new_post_document_id}"
                                                   type="hidden">
                                        </div>
                                    </li>
                                {/if}
                                {if $order->new_post_cost_delivery}
                                    <li class="reci_d new_post_remove">
                                        <div class="add_en_cost"><label>Цена:</label>
                                            <span><b>{$order->new_post_cost_delivery} грн.</b></span></div>
                                        <input name="new_post_cost_delivery" value="{$order->new_post_cost_delivery}"
                                               type="hidden">
                                    </li>
                                {/if}
                                {if $order->new_post_date_delivery}
                                    <li class="reci_d new_post_remove">
                                        <div class="add_en_data">
                                            <label>Дата: </label><span><b>{$order->new_post_date_delivery}</b></span>
                                        </div>
                                        <input name="new_post_date_delivery" value="{$order->new_post_date_delivery}"
                                               type="hidden">
                                    </li>
                                {/if}
                                <li class="reci_d"><label class=property>Город:</label>
                                    <input type="hidden" name="new_post_recipient_city_old"
                                           value="{$order->new_post_recipient_city}"/>
                                    <select name="new_post_recipient_city" class="simpla_inp">
                                        {if $new_post_city}
                                            {foreach $new_post_city as $city}
                                                <option ref="{$city['Ref']}"
                                                        {if $city['DescriptionRu'] == $order->new_post_recipient_city}selected{/if}
                                                        value="{$city['DescriptionRu']}">{$city['DescriptionRu']}</option>
                                            {/foreach}
                                        {else}
                                ...
                                <li class="reci_d"><label class=property>Отделения:</label>
                                    <input type="hidden" name="new_post_ware_house_old"
                                           value="{$order->new_post_recipient_ware_house}"/>
                                    <select name="new_post_recipient_ware_house" class="simpla_inp">
                                        {if $new_post_ware_house_list}
                                            {foreach $new_post_ware_house_list as $war_house}
                                                {if $war_house['TotalMaxWeightAllowed'] > $purchases_weight or $war_house['TotalMaxWeightAllowed']==''}
                                                    <option ref="{$war_house['Ref']}"
                                                            {if $war_house['DescriptionRu'] == $order->new_post_recipient_ware_house}selected{/if}
                                                            value="{$war_house['DescriptionRu']}">{$war_house['DescriptionRu']} </option>
                                                {/if}
                                            {/foreach}
                                        {else}

...
class OrderAdmin extends Simpla
{
	public function fetch()
	{
		...
		if($this->request->method('post'))
		{
            ...
            $order->new_post_document_id = $this->request->post('new_post_document_id', 'string');
            $order->new_post_cost_delivery = $this->request->post('new_post_cost_delivery');
            $order->new_post_date_delivery = $this->request->post('new_post_date_delivery');

            $order->new_post_name = $this->request->post('new_post_name');
            $order->new_post_midle_name = $this->request->post('new_post_midle_name');
            $order->new_post_last_name = $this->request->post('new_post_last_name');

            if($this->request->post('new_post_custom_weight')!=''){
                $order->new_post_custom_weight = $this->request->post('new_post_custom_weight');
            }

            $order->new_post_recipient_city = $this->request->post('new_post_recipient_city', 'string');
            $order->new_post_recipient_ware_house = $this->request->post('new_post_recipient_ware_house');
		...
		if($order && $purchases = $this->orders->get_purchases(array('order_id'=>$order->id)))
		{
			...
			foreach($purchases as &$purchase)
			{
                ...
                $purchases_weight += $purchase->variant->new_post_weight*$purchase->amount;
			}
        ...
        $this->design->assign('purchases_weight', $purchases_weight);

        require_once('./api/NovaPoshtaApi2.php');
        if($this->settings->new_post_key!='')
        {
            $np = new NovaPoshtaApi2($this->settings->new_post_key,
                'ru', // Язык возвращаемых данных: ru (default) | ua | en
                FALSE, // При ошибке в запросе выбрасывать Exception: FALSE (default) | TRUE
                'curl' // Используемый механизм запроса: curl (defalut) | file_get_content
            );
            $city = $np->getCities();
            $this->design->assign('new_post_city', $city['data']);

        }

        if($order->new_post_recipient_city!='' && $this->settings->new_post_key!=''){
            $cityRef = $np->getCities(0,$order->new_post_recipient_city);
            $new_post_ware_house=$np->getWarehouses($cityRef['data'][0]['Ref']);
            $this->design->assign('new_post_ware_house_list', $new_post_ware_house['data']);
        }
